	...
	function main(){
		if(isset($_GET['cmd'])){
			...
			foreach($result['data'] as $group=>$items){
				...
				$html =<<<EOF
	<style>
	#container p,#container td,#container div,#container th{line-height:32px; }
	#container{ margin:0px auto; text-align:left;}
	#main{}
	#setting{ width:100%;}
	#setting th{ text-align:left; padding-left:20px; font-weight:normal; }    
	tr.title{ background:#f9f9f9; color:#333; font-weight:700;  border-top:1px solid #CED5DA;  border-bottom:1px solid #CED5DA;}
	tr.title td{ padding:5px; }
