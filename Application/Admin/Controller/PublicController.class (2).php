...
class PublicController extends Controller {	
	...
	public function login(){
		...
		session('admin', $admin);
		session('[pause]');
		$updateData = array('update_time'=>time());
		$r = $adminMod->where($data)->data($updateData)->save();
		$this->redirect('index');
		return ;
	}
	 ...
	 */
	protected function ajaxEdit($modName, $template = null, $callback = null){
		$mod = d($modName);
		if($_POST){ 
			$data = $_POST;
			$id   = (int)$_POST['id'];
