...
<div class="table-responsive shrink count">
 <h2 class="sub-header">
  ...
  <?php else: ?>
  <table class="table table-striped num-list">
		<tr>
		  <th class="r1"></th>
		<?php if(is_array($fieldArr)): foreach($fieldArr as $key=>$v): ?><th><?php echo ($v[0]); ?></th><?php endforeach; endif; ?>
		</tr>
		<?php if(is_array($rows)): foreach($rows as $key=>$v): ?><tr>
			<?php if(is_array($v)): foreach($v as $k=>$v2): if($k > 0): ?><td <?php if($k == 1): ?>class="r1"<?php endif; ?> ><?php echo ($v2); ?></td><?php endif; endforeach; endif; ?>
