...
<style>
	#think_page_trace_open,nav,.sidebar{display:none;}
...
<div class="table-responsive shrink">
	<form class="form-inline search clearfix">
		...
		<div class="btn-group pull-left">
			<input  class="form-control" name="title" type="text" placeholder="任务标题" value="<?php echo ($search["title"]); ?>" > 
			<input name="task_id" value="<?php echo ($search["task_id"]); ?>" type="hidden" />
	...
	<table class="table table-striped artist-list">
		...
		<?php if(is_array($list)): foreach($list as $key=>$v): ?><tr>
			<td><?php echo ($v["id"]); ?></td>
			<td><?php if($v['user']['avatar']): ?><img src="<?php echo ($v["user"]["avatar"]); ?>" width=40 /><?php endif; ?></td>
			<td><?php echo ($v["user"]["nickname"]); ?></th>
			<td><?php echo ($v["task"]["title"]); ?></td>
			<td><?php echo ($v["task"]["typeName"]); ?></td>
			<td><?php echo ($v["task"]["cityName"]); ?></td>
			<td><?php echo ($v["task"]["styleName"]); ?></td>
			<td><?php echo ($v["model"]); ?></td>
			<td class="price"><?php echo ($v["price"]); ?></td>
			<td><?php echo ($v["num"]); ?></td>
			<td><?php echo ($v["task"]["beginTime"]); ?></td>
			<td><?php echo ($v["task"]["overTime"]); ?></td>
			<td><?php echo ($v["statusName"]); ?></td>
			<td width=120 class="handle hide">
				<?php if(1 > $v['status']): ?><a class="ajaxPost btn btn-warning btn-sm" href="#" url="<?php echo u('taskCancel');?>" data="id=<?php echo ($v["id"]); ?>" success="已取消">取消招标</a><?php endif; ?>
			</td>
