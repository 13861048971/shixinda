...
<div class="table-responsive shrink">
	...
	<table class="table table-striped artist-list">
		...
		<?php if(is_array($list)): foreach($list as $key=>$v): ?><tr>
			...
			<td><a class="btn-link" href="javascript:openInNewWindow('/Admin/Run/join?task_id=<?php echo ($v["id"]); ?>','任务“<?php echo ($v["title"]); ?>” 的投标')"><?php echo ($v["joinNum"]); ?></a></td>
			<td><?php echo ($v["statusName"]); ?></td>
			<td width=120 class="handle">
				<?php if(1 > $v['status']): ?><a class="ajaxPost btn btn-warning btn-sm" href="#" url="<?php echo u('taskCancel');?>" data="id=<?php echo ($v["id"]); ?>" confirm="true" success="已取消">取消招标</a><?php endif; ?>
			</td>
