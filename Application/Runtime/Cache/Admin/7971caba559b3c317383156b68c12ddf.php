...
<div class="table-responsive user">
	...
	<table class="table table-striped">
		...
		<?php if(is_array($list)): foreach($list as $key=>$v): ?><tr titie="<?php echo ($v["statusName"]); ?>">
			<td><?php echo ($v["id"]); ?></td>
			<td><?php if($v['image']): ?><a href="javascript:openInNewWindow('<?php echo ($v["image"]); ?>')">
						<img class="avatar" src="<?php echo ($v["image"]); ?>" width=25 />
					</a><?php endif; ?>
			</td>
			<td><?php echo ($v["typeName"]); ?></td>
			<td><?php if($v['type'] < 4 and $v['pho']['nickname']): echo ($v["pho"]["nickname"]); else: echo ($v["node"]["title"]); endif; ?>
				<?php if($v['type'] == 4): ?><a href="<?php echo ($v["link"]); ?>" target="_blank" class="btn-link">链接</a><?php endif; ?>
			</td>
			<td><?php echo ($v["rank"]); ?></td>
			<td><?php echo ($v["addTime"]); ?></td>
			<td><?php echo ($v["updateTime"]); ?></td>
			<td class="handle">
				<a class="dialog edit" href="#" url="<?php echo u('slideEdit','id=' . $v['id']);?>" dialogTitle="编辑轮播图">编辑</a> | 
				<a class="ajaxDel del" href="#" url="<?php echo u('slideDel','id=' . $v['id']);?>">删除</a>
			</td>
