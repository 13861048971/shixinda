...
jQuery(document).ready(function(){
  var SERVER_HOST = document.location.origin;
  $(document).on('click', '.file-uploader',function(){
	console.log('===');
    var _this   = $(this);
    var uploaderModal = _this.parents('.form-group').next();
    if (undefined == WebUploader) {
      console.error('请先加载webUploader.js');
    };
	uploaderModal.find(':submit').show();
    ...
    var fileList   = uploaderModal.find('#uploader-file-list'),
        picker     = uploaderModal.find('#uploader-picker'),
        controlBtn = uploaderModal.find('#uploader-ctlBtn'),
        state      = 'pending',
        uploader;
    ...
    uploader.on('uploadAccept',  function(object, ret) {
        ...
        if (ret.err == 0) {
          ...
          if ($(options.urlContainer).length) {
			...
			preview[0] && ret.thumb && (preview.attr('src', ret.thumb));
          };
